...
*/
    ...
    #define NPK_array_to_packet(type, arr) NPK_array_to_packet_(NPK_sizeof_type(type), arr)
    #define NPK_string_to_packet(str) NPK_array_to_packet_(1, str)

    #define NPK_packet_to_array(type, p) NPK_packet_to_array_(NPK_sizeof_type(type), p)
    ...
    void NPK_packet_address(UDPpacket *p, const NPK_cstring *ip, int port);
#endif
